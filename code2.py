...
import os
import asyncio
from bot import main as run_bot  # Импортируйте вашу функцию main из bot.py

app = Flask(__name__)

# Фоновый запуск бота в отдельном потоке
def start_bot():
    asyncio.run(run_bot())
...
if not TELEGRAM_TOKEN or not OPENROUTER_API_KEY:
    raise ValueError("Токены не найдены! Создайте файл .env с TELEGRAM_TOKEN и OPENROUTER_API_KEY")
...
async def analyze_literature(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Пробуем текущую модель
        analysis = await analyze_with_openrouter(query, current_model, timeout=40.0)
        
        # Если не получилось, пробуем другие модели
        if not analysis and available_models:
            for model in available_models:
                if model != current_model:
                    analysis = await analyze_with_openrouter(query, model, timeout=35.0)
                    if analysis:
                        current_model = model
                        break
